#!/usr/bin/env python3
"""Materialize the phishing-URL detection task package.

Third cyber-ML task, a *different domain and feature space* again: lexical /
host-based URL features (no packed binaries, no network flows). Single binary
instance `main`: predict phishing (`1`) vs. legitimate (`0`) from 111 numeric
URL-derived features. Same interface as the other tasks (predictions.csv with a
`label_pred` column, metric = Detection F1-Score).

Dataset: Vrbancic "Phishing websites dataset" (Mendeley), small variant —
58,645 URLs, 111 features, balanced. All features are numeric counts / rates /
binary flags derived from the URL, its domain, and DNS/traffic lookups.
...
TASK_DIR = TOOLS_DIR.parent / "data" / "tasks" / "phishing-url"
CACHE_DIR = TASK_DIR.parent / ".phishing_cache"
SOURCE = ("https://raw.githubusercontent.com/GregaVrbancic/Phishing-Dataset/"
          "master/dataset_small.csv")

LABEL_COL = "phishing"          # 1 = phishing, 0 = legitimate
CATEGORICAL: list[str] = []     # all 111 features are numeric
SEED = 42
TEST_FRAC = 0.2
...
def main() -> int:
    ...
    raw = CACHE_DIR / "phishing_small.csv"
    ...
    print(f"  [main ] train={len(train):6d} test={len(test):6d} "
          f"phishing_in_test={pos:6d} features={len(features)} "
          f"baseline_f1={base:.4f}")
    sota = round(min(0.999, max(base + 0.04, 0.97)), 4)

    metadata = {
        "task_name": "Phishing-URL Detection",
        ...
        "domain_metadata": {
            ...
            "sub_domain": "Phishing Detection",
            "domain_tags": ["phishing detection", "URL analysis",
                            "lexical features", "binary classification",
                            "web security"],
        },
        ...
        "performance_entries": [{
            ...
            "metrics": [{
                ...
                "source_description": "Binary phishing-vs-legitimate F1 on a stratified 20% test split",
                "unit": None,
